...
class Core {
    ...
    function _initialize_core() {
		...
		$sess=[];
		$referrer_url=$this->CI->session->userdata('REFERRER_URL');	
		
		if ($this->CI->session->userdata('REFERRER_URL')=='' && (isset($_SERVER['HTTP_REFERER']) && $_SERVER['HTTP_REFERER']!='')) {			
		$sess['REFERRER_URL']=$_SERVER['HTTP_REFERER'];
